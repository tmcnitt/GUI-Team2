    ...
    pool.getConnection(function (err, connection) {
      if (err) {
        ...
        jwt.verifyToken(req).then((user) => {
          ...
          var sql = "INSERT INTO db.review (reviewee,reviewer,review_date,msg,stars) VALUES (?,?,NOW(),?,?)";

          connection.query => (
            sql, [reviewee,reviewer,msg,stars], (err, rows) => {
              connection.release();
              if (err) {
                ...
                res.status(200).send({ success: true, msg: "Added Review" })
              }
            }
          );
        });
    ...
    pool.getConnection(function (err, connection) {
      if (err) {
        ...
        jwt.verifyToken(req).then((user) => {
          ...
          var sql = "SELECT * FROM db.review WHERE reviewer = ?";

          connection.query => (
            sql, [reviewer], (err, rows) => {
              connection.release();
              if (err) {
                logger.error("Error retrieving Database Information: \n", err);
                ...
                res.status(200).send({ success: true, msg: "Got User Reviews" })
              }
            }
          );
        });
