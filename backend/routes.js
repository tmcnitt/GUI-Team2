...
const { GET_AUCTIONS, GET_TRANSACTIONS, GET_FIXED } = require("./sql");
...
module.exports = function routes(app, logger) {
  ...
  app.get('/fixed/:id?', (req, res) => {
    jwt.verifyToken(req).then(() => {
      ...
      const sql = GET_FIXED + mod + " GROUP BY db.fixed_price.id HAVING product_name IS NOT NULL";
  ...
  app.get('/auctions/:id?', function (req, res) {
    jwt.verifyToken(req).then((user) => {
      ...
      // Convert the user id into a username for the table
      // Convert the reviews into an average score
      const sql = GET_AUCTIONS + mod + "  GROUP BY db.auction.id HAVING product_name IS NOT NULL";
